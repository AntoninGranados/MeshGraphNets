...
def display_trajectory(data: Mesh, meta: dict, max_frame: int|None = None, title: str = "", save: bool = False, save_path: Path|str = "") -> None:
    ...
    for i in range(frame_count):
        ...
        ax.set_xlim(max(np.min(world_pos[:,:,0]),-3), min(np.max(world_pos[:,:,0]), 3))
        ax.set_ylim(max(np.min(world_pos[:,:,1]),-3), min(np.max(world_pos[:,:,1]), 3))
        ax.set_zlim(max(np.min(world_pos[:,:,2]),-3), min(np.max(world_pos[:,:,2]), 3))
        ax.set_axis_off()
        ax.set_aspect('equal')

        ax.set_title("Prediction", color="gray", fontsize=12)
        ...
        fig.text(0.5, 0.80, f"Frame: {i+1:>4}/{frame_count}", ha="center", fontsize=10, color="white")
...
def display_trajectory_list(meshes: list[Mesh], names: list[str], meta: dict, title: str = "", loss: list = [], save: bool = False, save_path: Path|str = "") -> None:
    ...
    for i in range(frame_count):
        ...
        for a in range(len(meshes)):
            ...
            axs[a].set_title(f"{names[a]}", color="gray", fontsize=12)
        ...
        if loss != []:
            subtitle = f"Frame: {i+1:>3}/{frame_count}\nLoss (RMSE): {loss[i]:.4f}"
        fig.suptitle(f"{title}", fontsize=16, y=0.95)
        fig.text(0.5, 0.80, subtitle, ha="center", fontsize=12, color="white")
        fig.patch.set_facecolor('k')
        fig.patch.set_alpha(0.2)
