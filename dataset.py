...
class Dataset(torch.utils.data.Dataset[Mesh]):
    ...
    def __add_targets(self, curr: Mesh, prev: Mesh, targ: Mesh) -> Mesh:
        ...
        prev_mesh = {k: v for k,v in prev.items()}
        curr_mesh = {k: v for k,v in curr.items()}
        targ_mesh = {k: v for k,v in targ.items()}

        out["prev|world_pos"] = interpolate_field(curr_mesh, prev_mesh, prev_mesh["world_pos"])
        out["target|world_pos"] = interpolate_field(curr_mesh, targ_mesh, targ_mesh["world_pos"])
